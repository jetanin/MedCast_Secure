      ...
      <div className="preview-overlay no-print" onClick={closePreview}>
        <div className="preview-box" onClick={(e) => e.stopPropagation()}>
          <div className="preview-bar">
            ...
            <span style={{ flex: 1 }} />
            <button className="tab" onClick={downloadPreview}>⬇️ ดาวน์โหลด</button>
            <button className="tab" onClick={closePreview}>ปิด</button>
          </div>
          {preview.mime.startsWith("image/") ? (
            <img src={preview.url} alt="เอกสารที่เซ็น" className="preview-content" />
          ) : (
            <iframe src={preview.url} title="เอกสารที่เซ็น" className="preview-content" />
          )}
        </div>
      </div>
    )}
